    ...
    const handleDelete = async (id) => {
        if (await confirmDelete('Excluir Funcionário?')) {
            try {
                ...
                await showAlert('Excluído!', 'Funcionário removido.');
                loadEmployees();
            } catch (error) {
                ...
                <div className="bg-white rounded-lg shadow-md overflow-hidden">
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tr>
                            ...
                            <SortableTh label="Cargo" sortKey="role.name" />
                        ...
                        {employees?.length === 0 ? (
                            ...
                            employees?.map((employee) => (
                                <tr key={employee.id} className="hover:bg-gray-50">
                                    ...
                                    {/* CORREÇÃO AQUI: Acessa .name e usa fallback */}
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <button onClick={() => navigate(`/employees/edit/${employee.id}`)} className="text-primary-600 hover:text-primary-900 mr-3"><Edit className="w-5 h-5" /></button>
                                        <button onClick={() => handleDelete(employee.id)} className="text-red-600 hover:text-red-900"><Trash2 className="w-5 h-5" /></button>
                                    </td>
                                </tr>
                            ))
                        )}
